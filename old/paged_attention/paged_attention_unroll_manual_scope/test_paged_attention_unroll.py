...
# -----------------------------------------------------------------------------------------------------------
"""Paged attention unroll, manual-scope variant (Case1) — ported to V200-benchmark.

Same kernel set, function IDs, and signatures as ``paged_attention_unroll``; the
orchestration uses an explicit manual-scope dependency pattern. Kernels live
...
class TestPagedAttentionUnrollManualScope(SceneTestCase):
    ...
    def generate_args(self, params):
        ...
        specs = [Tensor(name, val) for name, val in inputs if isinstance(val, torch.Tensor)]
