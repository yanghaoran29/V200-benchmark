...
每个变体按 SPMD 用量分三档（全部不用 / 部分 / 全部 SPMD，详见 1.3）：三档共用同一套 kernel 与**同一份**参数化编排 `orchestration/qwen3_decode.cpp`，档位由编译宏 `-DQWEN3_SPMD_TIER=0/1/2` 在编译期传入（无需为每档单独建源文件）；`test_qwen3_decode.py` 内三个用例类各自声明档位，由测试框架注入到该编排的编译命令。
...
SPMD 用量直接决定编排压力：SPMD 用得越多，task 数越少、AICPU 逐 task 调度负担越轻；用得越少，task 数越多，越能压测 AICPU 在高编排压力下的表现。为覆盖这一范围，把 SPMD 用量设计成可配置的"分档"参数，分三档：

- **全部不用 SPMD**：每个可分块算子都逐块单任务下发，task 数最多，用于测**最大编排压力**下的 AICPU 性能。
- **部分 SPMD**：注意力（qk_matmul / softmax / sv_matmul / online_softmax）与 gate / up / silu 走 SPMD，其余逐块下发，贴近**实际折中**场景。
- **全部 SPMD**：每个可分块算子都用一次 SPMD 下发，task 数最少，用于测**最佳编排性能**。

### 1.3.1 参数化方式

设某算子共需并行执行 n 个子任务，取 SPMD 宽度 m（m 整除 n），按下式下发：

```
for i in range(n / m):
    func(spmd=m, chunk_base=i*m)   # 每次 SPMD 下发含 m 个子任务，共 n/m 次
```

- `m = n`：一次 SPMD 下发（`block_num = n`），即"用 SPMD"。
- `m = 1`：n 次单任务下发（`block_num = 1`），即"不用 SPMD"。

三档即对每个算子在 `{1, n}` 间取 m 的三组预设（见 1.3.2）。两种模式**共用同一份 kernel**：`kernel_entry` 内部把实际块号算成 `chunk_base + block_idx`，因此"一次 SPMD 下发 [chunk_base=0, block_num=n]"与"逐块下发 [chunk_base=i, block_num=1]"走同一份 kernel，差异只落在编排的下发循环上。

档位在编译期由宏 `QWEN3_SPMD_TIER`（0 / 1 / 2）选择。每个变体只有**一份**编排源 `orchestration/qwen3_decode.cpp`（默认 `QWEN3_SPMD_TIER=2`）；三个档位通过 `-DQWEN3_SPMD_TIER=<档位>` 编译选项区分，不再为每档单建源文件。`test_qwen3_decode.py` 内三个用例类各自声明档位（`CALLABLE["orchestration"]["spmd_tier"]`），由测试框架注入到该编排的编译命令。

### 1.3.2 各算子分档参数（每算子 SPMD 宽度 m）

| Scope | 算子 | 类型 | n（子任务数） | 全不用 m | 部分 m | 全部 m |
|-------|------|------|--------------|---------|--------|--------|
| **Scope1** | input rmsnorm | AIV | 1 | — | — | — |
| | q_proj | AIC | 20 | 1 | 1 | 20 |
| | k_proj | AIC | 8 | 1 | 1 | 8 |
| | v_proj | AIC | 8 | 1 | 1 | 8 |
| | qk_norm | AIV | 1 | — | — | — |
| **Scope2** | rope_kv_cache | AIV | 1（每 batch） | — | — | — |
| | qk_matmul | AIC | 4 | 1 | 4 | 4 |
| | softmax | AIV | 4 | 1 | 4 | 4 |
| | sv_matmul | AIC | 4 | 1 | 4 | 4 |
| | online_softmax | AIV | 4 | 1 | 4 | 4 |
| **Scope3** | out_proj（mixed） | MIX | 40 | 1 | 1 | 40 |
| | post_rmsnorm | AIV | 1 | — | — | — |
| | gate_proj | AIC | 34 | 1 | 34 | 34 |
| | up_proj | AIC | 34 | 1 | 34 | 34 |
| | silu | AIV | 34 | 1 | 34 | 34 |
| | down_proj | AIC | 40 | 1 | 1 | 40 |
| | down_proj_residual | AIV | 40 | 1 | 1 | 40 |

注：
- rmsnorm / qk_norm / rope_kv_cache / post_rmsnorm 为不可分块的单任务算子，不随档位变化（表中以 — 标注）。
- `m = 1` 即"不用 SPMD"，该算子按 n 次单任务逐块下发；`m = n` 即"用 SPMD"，一次下发。
- 注意力四算子与 gate/up/silu 在"部分""全部"两档均取 `m = n`（满 SPMD），仅在"全不用"档退化为 `m = 1`。

档位对编排压力的影响可由提交 task 数直观体现（manual_scope，Case1，a2a3sim 实测）：**全部不用 SPMD 3205 task、部分 SPMD 1531 task、全部 SPMD 631 task**。
...
2. 三档（全部不用 / 部分 / 全部 SPMD）合并为同一套 kernel + 同一份参数化编排：`kernel_entry` 增加 `chunk_base` 标量，实际块号取 `chunk_base + block_idx`，使"单次 SPMD 下发"与"逐块下发"共用 kernel；编排按编译期宏 `QWEN3_SPMD_TIER` 选档，每变体仅一份编排源 `qwen3_decode.cpp`，档位通过 `-DQWEN3_SPMD_TIER=0/1/2` 编译选项传入（测试侧三个用例类声明档位、由框架注入编译命令），不再为每档单建源文件；新增"全部不用 SPMD"档。三档 golden 在 a2a3sim 均通过（manual_scope 提交 task 数分别为 3205 / 1531 / 631）。
3. 重写 1.3「SPMD 方案设计」：改为分档参数化表述（n / 各档 m / 下发次数），三档合并为一张统一口径表并补充实测 task 数；1.1.1 补充三档说明。
