...
SPMD 用量直接决定编排压力：SPMD 用得越多，task 数越少、AICPU 逐 task 调度负担越轻；用得越少，task 数越多，越能压测 AICPU 在高编排压力下的表现。为覆盖这一范围，把 SPMD 用量设计成可配置的"分档"参数，对**所有**可分块算子施加**统一**的目标 SPMD 宽度 m（按各算子 total_chunks 封顶），分五档：

| 档位 | CLI 参数 | `QWEN3_SPMD_TIER` | 目标宽度 m | 含义 |
|------|---------|-------------------|-----------|------|
| 全部不用 SPMD | `--non-spmd` | 0 | 1 | 每个可分块算子逐块单任务下发，task 数最多 → 测**最大编排压力** |
| SPMD 宽度 2 | `--spmd-2` | 1 | 2 | 每算子每 2 块一次 SPMD 下发 |
| SPMD 宽度 4 | `--spmd-4` | 2 | 4 | 每算子每 4 块一次 SPMD 下发 |
| SPMD 宽度 8 | `--spmd-8` | 3 | 8 | 每算子每 8 块一次 SPMD 下发 |
| 全部 SPMD | `--all-spmd` | 4（默认） | total_chunks | 每算子一次 SPMD 下发，task 数最少 → 测**最佳编排性能** |

与旧"部分 SPMD"（只对注意力等部分算子开 SPMD）不同，新分档对**所有**可分块算子使用同一目标宽度，仅按各算子可用 chunk 数封顶——例如注意力四算子只有 4 个 chunk，在 `--spmd-4` / `--spmd-8` / `--all-spmd` 各档都只发 4 个 block。

### 1.3.1 各算子分档参数（每算子实际 SPMD 宽度 = min(目标 m, n)）

| Scope | 算子 | 类型 | n（子任务数） | non-spmd | spmd-2 | spmd-4 | spmd-8 | all-spmd |
|-------|------|------|--------------|:--------:|:------:|:------:|:------:|:--------:|
| **Scope1** | input rmsnorm | AIV | 1 | — | — | — | — | — |
| | q_proj | AIC | 20 | 1 | 2 | 4 | 8 | 20 |
| | k_proj | AIC | 8 | 1 | 2 | 4 | 8 | 8 |
| | v_proj | AIC | 8 | 1 | 2 | 4 | 8 | 8 |
| | qk_norm | AIV | 1 | — | — | — | — | — |
| **Scope2** | rope_kv_cache | AIV | 1（每 batch） | — | — | — | — | — |
| | qk_matmul | AIC | 4 | 1 | 2 | 4 | 4 | 4 |
| | softmax | AIV | 4 | 1 | 2 | 4 | 4 | 4 |
| | sv_matmul | AIC | 4 | 1 | 2 | 4 | 4 | 4 |
| | online_softmax | AIV | 4 | 1 | 2 | 4 | 4 | 4 |
| **Scope3** | out_proj（mixed） | MIX | 40 | 1 | 2 | 4 | 8 | 40 |
| | post_rmsnorm | AIV | 1 | — | — | — | — | — |
| | gate_proj | AIC | 34 | 1 | 2 | 4 | 8 | 34 |
| | up_proj | AIC | 34 | 1 | 2 | 4 | 8 | 34 |
| | silu | AIV | 34 | 1 | 2 | 4 | 8 | 34 |
| | down_proj | AIC | 40 | 1 | 2 | 4 | 8 | 40 |
| | down_proj_residual | AIV | 40 | 1 | 2 | 4 | 8 | 40 |

注：
- 表中数字为各算子的实际 SPMD 宽度 `min(目标 m, n)`；目标宽度超过该算子 chunk 数时按 n 封顶（如注意力四算子封顶为 4）。
- rmsnorm / qk_norm / rope_kv_cache / post_rmsnorm 为不可分块的单任务算子，不随档位变化（表中以 — 标注）。
- `min = 1` 即该算子逐块单任务下发；`min = n` 即一次 SPMD 全量下发。
- 各档位下"目标宽度 m → 每算子实际下发"的换算规则见 1.3.2。

### 1.3.2 参数化方式

设某算子共需并行执行 n 个子任务，目标宽度 m 对该算子取 `min(m, n)`，按下式下发：

```
m_op = min(m, n)
for base in range(0, n, m_op):
    func(spmd=min(m_op, n - base), chunk_base=base)   # 每次 SPMD 下发 ≤ m_op 个子任务
```

- `m = total_chunks`（`--all-spmd`）：一次 SPMD 下发（`block_num = n`），即"用 SPMD"。
- `m = 1`（`--non-spmd`）：n 次单任务下发（`block_num = 1`），即"不用 SPMD"。
- 中间档 `m ∈ {2,4,8}`：每 m 块一次 SPMD 下发，共 `ceil(n/m)` 次；当 m 不整除 n 时，最后一次发较小的 tail `block_num`。

五档**共用同一份 kernel**：`kernel_entry` 内部把实际块号算成 `chunk_base + block_idx`，因此"一次 SPMD 下发 [chunk_base=0, block_num=n]"与"逐块下发 [chunk_base=i, block_num=1]"走同一份 kernel，差异只落在编排的下发循环上。

档位在编译期由宏 `QWEN3_SPMD_TIER`（0…4）选择。每个变体只有**一份**编排源 `orchestration/qwen3_decode.cpp`（默认 `QWEN3_SPMD_TIER=4` 即 `--all-spmd`）；五个档位通过 `-DQWEN3_SPMD_TIER=<档位>` 编译选项区分，不再为每档单建源文件。`test_qwen3_decode.py` 内五个用例类各自声明档位（`CALLABLE["orchestration"]["spmd_tier"]`），由测试框架注入到该编排的编译命令；standalone 运行时可直接用上表的 CLI 参数（`--non-spmd` / `--spmd-2` / `--spmd-4` / `--spmd-8` / `--all-spmd`）选择对应档位（框架把该 flag 翻译成对应用例类的 `--case` 选择，不带 flag 时五档全跑），例如：

```
python test_qwen3_decode.py -p a2a3sim --non-spmd
python test_qwen3_decode.py -p a2a3  --all-spmd --enable-l2-swimlane
```

### 1.3.3 各档 kernel 下发次数（编排压力对比）

下表为按 1.3.1 公式推导的 kernel 下发次数（batch=90 → batch_padded=96，Scope1/3 各 6 个 tile、Scope2 逐 batch；不含 alloc 等框架开销 task）。下发次数随档位单调下降，直观体现编排压力的递减：
...
注：
- `--non-spmd` 合计 3096 与 1.2.1 的 TOTAL 子任务实例数一致（逐块单任务下发时，下发次数即子任务实例数）。
- Scope2 在 `--spmd-4` 及以上不再变化（注意力四算子已封顶为 4，rope 为单任务）。
- 框架最终提交的 task 数（含 alloc/编排 task）高于上表的纯 kernel 下发次数，具体数值以各档 a2a3sim 实测为准。
...
2. 按 SPMD 用量分的多档合并为同一套 kernel + 同一份参数化编排，并定为**五档统一宽度**：`kernel_entry` 增加 `chunk_base` 标量，实际块号取 `chunk_base + block_idx`，使"单次 SPMD 下发"与"逐块下发"共用 kernel；对所有可分块算子施加同一目标宽度 `m`（按各算子 chunk 数封顶），`kSpmdTargets={1,2,4,8,∞}`，编译期宏 `QWEN3_SPMD_TIER`（0…4，默认 4 = `--all-spmd`）选档；每变体仅一份编排源 `qwen3_decode.cpp`，五档通过 `-DQWEN3_SPMD_TIER=<档位>` 编译选项区分，不再为每档单建源文件。两 `test_qwen3_decode.py` 改为五个用例类各自声明档位（`CALLABLE["orchestration"]["spmd_tier"]`），并在 standalone `__main__` 中把 `--non-spmd` / `--spmd-2` / `--spmd-4` / `--spmd-8` / `--all-spmd` 翻译成对应 `--case`（框架固定 argparse 不识别自定义 flag）。
3. 重写 1.3「SPMD 方案设计」：五档统一宽度分档表（每算子实际宽度 `min(m,n)`）+ 各档 kernel 下发次数对比（non/2/4/8/all = 3096 / 1602 / 864 / 678 / 522）；1.1.1 同步五档说明。
4. 统一编排各算子头注释为 `// Task N: 名称` 形式（不再写 `// Spmd ..._spmd:`）。
