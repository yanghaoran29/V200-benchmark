...
1. **减少调度空隙**：SPMD 的 `block_num` 不宜过大；**不同 Scope 的外层循环 iteration 不合并为一次 SPMD**（例如 6 个 tile 的 q_proj 是 6 次独立 SPMD launch，而非 block_num=120）。
2. **V200 Orchestrator 压力**：V200 的 AICore 核数远多于 A2/A3，能在 SPMD 处并行展开的算子优先 SPMD，以减轻 Orchestrator 逐 task 调度负担。

### 1.3.2 各 Scope SPMD 使用明细（代码实现）
