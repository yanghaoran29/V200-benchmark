...
# Each token retains five disjoint cache-page splits.
REDUCE_NSPLIT = 5
# Pinned SPMD width per batch_tile (grid-stride over T_tile * REDUCE_NSPLIT);
# recommended lt12/ht60 have 3 tiles → 120.
SCORE_BLOCKS = 40
Q_TILE = 256
# Q_OUT_TILE is the per-task N granularity (sets idx_qr_proj task count); MM_N_TILE
# is the Mat-safe cube N-tile. Each task covers Q_OUT_TILE // MM_N_TILE cube ops.
Q_OUT_TILE = 1024
MM_N_TILE = 512
MM_ROW_TILE = 16
T_PAD = ((T + MM_ROW_TILE - 1) // MM_ROW_TILE) * MM_ROW_TILE
# Scheme B processes every padded M tile inside the existing dispatch.
HEAD_DIM_TILE = 32
D_TILE = 512
...
def indexer(
    ...
    n_idx_n = IDX_N_HEADS * IDX_HEAD_DIM // Q_OUT_TILE
    for tile_i in pl.range(N_BATCH_TILES):
        for ot_n in pl.spmd(n_idx_n, name_hint="idx_qr_proj_matmul"):
            o_base = ot_n * Q_OUT_TILE
            t_base = tile_i * T_TILE_BATCH
            for row0_local in pl.range(0, T_TILE_BATCH, MM_ROW_TILE):
                row0 = t_base + row0_local
                n_rows = pl.min(MM_ROW_TILE, T_TILE_BATCH - row0_local)
                for ns in pl.range(0, Q_OUT_TILE, MM_N_TILE):
                    ...
                    for kb in pl.pipeline(0, Q_LORA // Q_TILE, stage=2):
                        q0 = kb * Q_TILE
                        qr_tile = pl.slice(qr, [MM_ROW_TILE, Q_TILE], [row0, q0], valid_shape=[n_rows, Q_TILE])
                        wq_tile = wq_b[q0 : q0 + Q_TILE, o_base + ns : o_base + ns + MM_N_TILE]
                        qr_acc = pl.matmul_acc(qr_acc, qr_tile, wq_tile, init_cond=(q0 == 0))
                    qr_acc_pad = pl.assemble(qr_acc_pad, qr_acc, [row0, o_base + ns])
    qr_proj = pl.create_tensor([T, IDX_N_HEADS * IDX_HEAD_DIM], dtype=pl.FP32)
    for tile_i in pl.range(N_BATCH_TILES):
        for ot_n in pl.spmd(n_idx_n, name_hint="idx_qr_proj_dequant"):
            o_base = ot_n * Q_OUT_TILE
            t_base = tile_i * T_TILE_BATCH
            wq_scale = pl.reshape(wq_b_scale[o_base : o_base + Q_OUT_TILE], [1, Q_OUT_TILE])
            for row0_local in pl.range(0, T_TILE_BATCH, 8):
                row0 = t_base + row0_local
                acc_fp32 = pl.cast(qr_acc_pad[row0:row0 + 8, o_base : o_base + Q_OUT_TILE], target_type=pl.FP32, mode="none")
                qr_dequant = pl.col_expand_mul(pl.row_expand_mul(acc_fp32, qr_scale[row0:row0 + 8, :]), wq_scale)
                qr_proj[row0:row0 + 8, o_base : o_base + Q_OUT_TILE] = qr_dequant
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="qr_rope_swap_idx"):
    ...
    ROPE_ITEMS_TILE = T_TILE_BATCH * IDX_N_HEADS // ROPE_ROW_TILE
    for tile_i in pl.range(N_BATCH_TILES):
        for idx_core in pl.spmd(16, name_hint="qr_rope"):
            o_base_t = tile_i * T_TILE_BATCH * IDX_N_HEADS
            for idx in pl.range(idx_core, ROPE_ITEMS_TILE, 16):
                o0 = o_base_t + idx * ROPE_ROW_TILE
    ...
    QH_MM_ITEMS_TILE = T_TILE_BATCH * IDX_N_HEADS // QH_MM_TILE
    for tile_i in pl.range(N_BATCH_TILES):
        for idx_core in pl.spmd(32, name_hint="qr_hadamard_matmul"):
            o_base_t = tile_i * T_TILE_BATCH * IDX_N_HEADS
            for idx in pl.range(idx_core, QH_MM_ITEMS_TILE, 32):
                o0 = o_base_t + idx * QH_MM_TILE
    ...
    QH_Q_ITEMS_TILE = T_TILE_BATCH * IDX_N_HEADS // QH_QUANT_TILE
    for tile_i in pl.range(N_BATCH_TILES):
        for idx_core in pl.spmd(8, name_hint="qr_hadamard_quant"):
            o_base_t = tile_i * T_TILE_BATCH * IDX_N_HEADS
            for idx in pl.range(idx_core, QH_Q_ITEMS_TILE, 8):
                o0 = o_base_t + idx * QH_QUANT_TILE
    ...
    # The caller supplies the input readiness dependency.
    for tile_i in pl.range(N_BATCH_TILES):
        with pl.spmd(WEIGHTS_OK, name_hint="weights_proj", deps=[late_dep]) as _weights_tid:
            ...
            for row0_local in pl.range(0, T_TILE_BATCH, MM_ROW_TILE):
                ...
                for db in pl.range(WEIGHTS_K_SLICE // D_TILE):
                    d0 = k_base + db * D_TILE
                    x_tile = pl.slice(x_flat, [MM_ROW_TILE, D_TILE], [row0, d0], valid_shape=[n_rows, D_TILE])
                    weights_proj_tile = weights_proj[d0 : d0 + D_TILE, :]
                    weights_acc = pl.matmul_acc(weights_acc, x_tile, weights_proj_tile, init_cond=(db == 0))
                weights_partial = pl.assemble(weights_partial, weights_acc, [kb * T_PAD + row0, 0])

    with pl.spmd(N_BATCH_TILES, name_hint="weights_proj_reduce"):
        tile_i = pl.tile.get_block_idx()
        t_base = tile_i * T_TILE_BATCH
        for row0_local in pl.range(0, T_TILE_BATCH, MM_ROW_TILE):
            row0 = t_base + row0_local
            w_sum = weights_partial[row0:row0 + MM_ROW_TILE, :]
            for kb in pl.unroll(1, WEIGHTS_OK):
                w_sum = pl.add(w_sum, weights_partial[kb * T_PAD + row0 : kb * T_PAD + row0 + MM_ROW_TILE, :])
            weights[row0:row0 + MM_ROW_TILE, :] = pl.mul(w_sum, WEIGHTS_SCALE)
    ...
    SCORE_ITEMS_TILE = T_TILE_BATCH * REDUCE_NSPLIT
    for tile_i in pl.range(N_BATCH_TILES):
        t_base = tile_i * T_TILE_BATCH
        for score_core in pl.spmd(SCORE_BLOCKS, name_hint="score",
                            optimizations=[pl.split(pl.SplitMode.NONE, slot_num=2)]):
            for unit_local in pl.range(score_core, SCORE_ITEMS_TILE, SCORE_BLOCKS):
                tg_local = unit_local // REDUCE_NSPLIT
                split = unit_local - tg_local * REDUCE_NSPLIT
                tg = t_base + tg_local
                b = tg // S
                s = tg - b * S
    ...
    for tile_i in pl.range(N_BATCH_TILES):
        for t_core in pl.spmd(8, name_hint="topk"):
            t_base = tile_i * T_TILE_BATCH
            for t_local in pl.range(t_core, T_TILE_BATCH, 8):
                t = t_base + t_local
...
def indexer_test(
    ...
    # Standalone: an invalid TaskId represents the absence of an RMS producer.
    ready_tids = pl.array.create(1, pl.TASK_ID)
    late_dep = ready_tids[0]
...
def build_tensor_specs(start_pos=None):
    ...
    return [
        ...
        TensorSpec("idx_kv_cache", [IDX_CACHE_BLOCK_NUM, IDX_STORAGE_BLOCK_SIZE, 1, IDX_HEAD_DIM], torch.int8, init_value=lambda: idx_kv_i8),
        TensorSpec("idx_kv_scale", [IDX_CACHE_BLOCK_NUM, IDX_STORAGE_BLOCK_SIZE, 1, 1], torch.float32, init_value=lambda: idx_kv_sc),
