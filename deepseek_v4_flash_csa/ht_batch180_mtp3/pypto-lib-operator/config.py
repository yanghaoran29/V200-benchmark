...
from serving_load import ds_batch_seq, flash_batch_tile
...
DECODE_BATCH = 20                  # B: requests per decode step (overridden by V200_SERVING_CASE)
DECODE_SEQ = 2                    # S: [previous, current] tokens per serving step
DECODE_BATCH, DECODE_SEQ = ds_batch_seq(DECODE_BATCH, DECODE_SEQ)
BATCH_TILE = flash_batch_tile(decode_batch=DECODE_BATCH)
assert DECODE_BATCH % BATCH_TILE == 0, (
    f"DECODE_BATCH={DECODE_BATCH} must be a multiple of BATCH_TILE={BATCH_TILE}"
)
N_BATCH_TILES = DECODE_BATCH // BATCH_TILE
T_TILE_BATCH = BATCH_TILE * DECODE_SEQ  # token rows per task tile (HT=80, LT=40)
