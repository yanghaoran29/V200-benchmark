...
Aligned with ``pypto-lib/models/deepseek_v4_flash_mtp`` (tiling / SPMD).
Only DECODE_BATCH / DECODE_SEQ are overridden by V200 serving case
``BASIC_BATCH4_MTP1`` (B=4 S=2); there is no batch-tile (N_BATCH_TILES) fork.
"""
...
from serving_load import ds_batch_seq
...
DECODE_BATCH = 8                  # B: requests per decode step (T=B*S fills MATMUL_T_TILE)
DECODE_SEQ = 2                    # S: [previous, current] tokens per serving step
# Leaf default / --serving-case BASIC_BATCH4_MTP1 → B=4 S=2 (same shape as mainline control).
