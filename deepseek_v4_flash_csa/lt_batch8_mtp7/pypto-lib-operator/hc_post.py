...
def hc_post(
    ...
    n_hc_post_tile = (T_TILE_BATCH // T_TILE) * HC_MULT
    for tile_i in pl.range(N_BATCH_TILES):
        for core in pl.spmd(8, name_hint="hc_post"):
            t_base = tile_i * T_TILE_BATCH
            for block_local in pl.range(core, n_hc_post_tile, 8):
                token_block_local = block_local // HC_MULT
                out_h = block_local % HC_MULT
                t0 = t_base + token_block_local * T_TILE
