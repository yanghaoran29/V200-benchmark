...
QPROJ_MM_N_TILE = 512    # qproj output-column tile (align flash_mtp)
...
def rope_prepare(
    ...
    token_tiles_tile = T_TILE_BATCH // Q_ROPE_T_TILE
    for qrp_idx_lane in pl.spmd(N_BATCH_TILES, name_hint="q_rope_prepare"):
        tile_i = qrp_idx_lane
        t_base = tile_i * T_TILE_BATCH
        for qrp_idx in pl.range(token_tiles_tile):
            qrp_t0 = t_base + qrp_idx * Q_ROPE_T_TILE
...
def q_proj_rope(
    ...
    for tile_base in pl.range(0, t_dim, PREFILL_DENSE_TILE):
        ...
        with pl.scope():
            ...
            for tg_idx in pl.spmd(qr_token_tiles, name_hint="qr_rms_norm_quant"):
                tg = tg_idx * T_TILE
...
def kv_proj_rope(
    ...
    for tile_base in pl.range(0, t_dim, PREFILL_DENSE_TILE):
        ...
        with pl.scope():
            ...
            # Honor the caller-provided input readiness dependency.
...
def qkv_proj_rope(
    ...
    # eight N-groups; QR_OK=8 expands them to 64 cube blocks and atomic-adds the
    ...
    n_qr_m_tile = (T_TILE_BATCH + QR_M_TILE - 1) // QR_M_TILE
    n_qproj_m_tile = (T_TILE_BATCH + QPROJ_M_TILE - 1) // QPROJ_M_TILE
    n_kv_m_tile = (T_TILE_BATCH + KV_M_TILE - 1) // KV_M_TILE
    n_qr_rms_tile = T_TILE_BATCH // T_TILE
    n_q_rope_t = T_TILE_BATCH // Q_ROPE_T_TILE
    n_kv_rms_t = T_TILE_BATCH // KV_RMS_T_TILE
    ...
    n_qr_spmd = (Q_LORA // QR_N_TILE) * QR_OK
    for tile_i in pl.range(N_BATCH_TILES):
        for qbg in pl.spmd(n_qr_spmd, name_hint="qr_proj_matmul"):
            q_a_col0 = (qbg // QR_OK) * QR_N_TILE
            qr_k_base = (qbg % QR_OK) * QR_K_SLICE
            t_base = tile_i * T_TILE_BATCH
            for tc in pl.range(n_qr_m_tile):
                t0_local = tc * QR_M_TILE
                t0 = t_base + t0_local
                q_acc = pl.create_tensor([QR_M_TILE, QR_N_TILE], dtype=pl.FP32)
                for db in pl.pipeline(QR_K_SLICE // QR_K_TILE, stage=2):
                    qr_d0 = qr_k_base + db * QR_K_TILE
                    qr_rows = pl.min(QR_M_TILE, T_TILE_BATCH - t0_local)
    ...
    # Two passes per block: pass 1 computes amax; pass 2 recomputes norm and quantizes.
    for tg_idx_lane in pl.spmd(N_BATCH_TILES, name_hint="qr_rms_norm_quant"):
        tile_i = tg_idx_lane
        t_base = tile_i * T_TILE_BATCH
        for tg_idx in pl.range(n_qr_rms_tile):
            tg = t_base + tg_idx * T_TILE
            qr_sq_sum = pl.full([1, T_TILE], dtype=pl.FP32, value=0.0)
            qr_amax_g = pl.full([1, T_TILE], dtype=pl.FP32, value=0.0)
    ...
    n_qproj_spmd = (H * HEAD_DIM) // QPROJ_MM_N_TILE
    for tile_i in pl.range(N_BATCH_TILES):
        for qn in pl.spmd(n_qproj_spmd, name_hint="qproj_matmul"):
            w_col0 = qn * QPROJ_MM_N_TILE
            t_base = tile_i * T_TILE_BATCH
            for tc in pl.range(n_qproj_m_tile):
                t0_local = tc * QPROJ_M_TILE
                t0 = t_base + t0_local
                ...
                for qb in pl.pipeline(0, Q_LORA // Q_PROJ_TILE, stage=2):
                    ...
                    qr_i8_chunk = pl.slice(qr_i8_matmul, [QPROJ_M_TILE, Q_PROJ_TILE], [t0, qr_proj_col0], valid_shape=[pl.min(QPROJ_M_TILE, T_TILE_BATCH - t0_local), Q_PROJ_TILE])
                    wq_chunk = wq_b[qr_proj_col0 : qr_proj_col0 + Q_PROJ_TILE, w_col0 : w_col0 + QPROJ_MM_N_TILE]
                    col_acc = pl.matmul_acc(col_acc, qr_i8_chunk, wq_chunk, init_cond=(qr_proj_col0 == 0))
    ...
    # RoPE: out[j] = inv_rms * (x[j] * cos[j] + x[j^1] * sign[j] * sin[j]).
    q_flat = pl.reshape(q, [t_dim, H * HEAD_DIM])
    n_q_rope_h = H // Q_ROPE_H_TILE
    q_rope_tid = None
    for tile_i in pl.range(N_BATCH_TILES):
        with pl.spmd(n_q_rope_h, name_hint="qproj_dequant_rms_nope_rope") as q_rope_tid:
            hg_idx = pl.tile.get_block_idx()
            hg = hg_idx * Q_ROPE_H_TILE
            t_base = tile_i * T_TILE_BATCH
            for tg_idx in pl.range(n_q_rope_t):
                tg = t_base + tg_idx * Q_ROPE_T_TILE
    ...
    # Split-K kv_proj uses four 128-column N-groups and KV_OK=8, producing
    # 32 cube blocks.
    kv_fp32 = pl.create_tensor([t_matmul, HEAD_DIM], dtype=pl.FP32)
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="kv_proj_seed"):
    ...
    # Honor the caller-provided input readiness dependency.
    n_kv_spmd = (HEAD_DIM // KV_N_TILE) * KV_OK
    for tile_i in pl.range(N_BATCH_TILES):
        with pl.spmd(n_kv_spmd, name_hint="kv_proj_matmul", deps=[late_dep]) as _kv_tid:
            kbg = pl.tile.get_block_idx()
            kv_col0 = (kbg // KV_OK) * KV_N_TILE
            kv_k_base = (kbg % KV_OK) * KV_K_SLICE
            t_base = tile_i * T_TILE_BATCH
            for tc in pl.range(n_kv_m_tile):
                t0_local = tc * KV_M_TILE
                t0 = t_base + t0_local
                kv_acc = pl.create_tensor([KV_M_TILE, KV_N_TILE], dtype=pl.FP32)
                for db in pl.pipeline(KV_K_SLICE // KV_K_TILE, stage=2):
                    d0 = kv_k_base + db * KV_K_TILE
                    kv_rows = pl.min(KV_M_TILE, T_TILE_BATCH - t0_local)
    ...
    for tg_idx_lane in pl.spmd(N_BATCH_TILES, name_hint="kv_rms_norm_rope"):
        tile_i = tg_idx_lane
        t_base = tile_i * T_TILE_BATCH
        for tg_idx in pl.range(n_kv_rms_t):
            tg = t_base + tg_idx * KV_RMS_T_TILE
