...
QK_STORE_H_TILE = 16
MERGE_H_TILE = 8
QK_M_TILE = 32           # qk_pv M rows per QK/PV matmul
ATTN_K_TILE = 128
NUM_QK_CORES = 40         # pinned per batch_tile; recommended 3 tiles → 120
A_K_TILE = 256           # proj_a cube K frag
PROJ_A_MM_N_TILE = 128   # proj_a cube N frag (align flash_mtp)
MM_T_TILE = 16
...
QUANT_TOKEN_TILE = 8
PROJ_B_D_TILE = 512      # proj_b_mm D chunk per task; its N frags loop inside the task (align flash_mtp)
...
QK_ITEMS_TILE = T_TILE_BATCH * SPARSE_BLOCKS  # items per batch tile
MERGE_ITEMS_TILE = T_TILE_BATCH * (H // MERGE_H_TILE)
PROJ_B_ACT_T_BLOCKS_TILE = T_TILE_BATCH // PROJ_B_ACT_TASK_T_TILE
assert T_TILE_BATCH % QUANT_TOKEN_TILE == 0
assert T_TILE_BATCH % PROJ_B_ACT_TASK_T_TILE == 0
...
def sparse_attn_csa(
    ...
    # One SPMD block per batch tile (not n_base*N fat grid).
    with pl.spmd(N_BATCH_TILES, name_hint="kv_touch"):
        touch_tile = pl.tile.get_block_idx()
        touch_t0 = touch_tile * T_TILE_BATCH
        ori_kv_flat[touch_t0:touch_t0 + T_TILE_BATCH, 0:HEAD_DIM] = ori_kv_flat[touch_t0:touch_t0 + T_TILE_BATCH, 0:HEAD_DIM]

    # Build validity metadata; QK/PV uses the natural token/block order.
    ...
    with pl.spmd(N_BATCH_TILES, name_hint="csa_slots_build_valid_qk_plan") as qk_plan_tid:
        plan_tile = pl.tile.get_block_idx()
        plan_t_base = plan_tile * T_TILE_BATCH
        for token0_local in pl.range(0, T_TILE_BATCH, 8):
            token0 = plan_t_base + token0_local
            c_raw = pl.cast(idx_topk[token0:token0 + 8, 0:IDX_TOPK], target_type=pl.FP32)
            c_pos = pl.cast(position_ids[token0:token0 + 8, 0:1], target_type=pl.FP32)
            ...
            c_upper_b = pl.row_expand_mul(pl.full([8, IDX_TOPK], dtype=pl.FP32, value=1.0), c_pos_q)
            ...
            cmp_sparse_indices[token0:token0 + 8, 0:IDX_TOPK] = pl.cast(c_out, target_type=pl.INT32)
            # Block 0 (sliding-window) is always live; blocks 1.. from the compressed mask.
            for c_t0 in pl.range(8):
                pl.write(valid_block_mask, [token0 + c_t0, 0], pl.cast(1, pl.INT32))
            for c_sb in pl.range(1, SPARSE_BLOCKS):
                ...
                for c_dt in pl.range(8):
                    ...
                    pl.write(valid_block_mask, [token0 + c_dt, c_sb], c_valid)
            ...
            v_win_f = pl.cast(window_swa_indices[token0:token0 + 8, 0:WIN], target_type=pl.FP32)
            ...
            sparse_bias[token0:token0 + 8, 0:WIN] = pl.mul(pl.sub(v_win_valid, 1.0), -NEG_INF)
            sparse_bias[token0:token0 + 8, WIN:TOPK] = pl.mul(pl.minimum(c_out, 0.0), -NEG_INF)
            if PADDED_TOPK > TOPK:
                sparse_bias[token0:token0 + 8, TOPK:PADDED_TOPK] = pl.full([8, PADDED_TOPK - TOPK], dtype=pl.FP32, value=NEG_INF)

    # One lane per core. Each lane walks its indexed items and gathers the
    ...
    # Outer range(tile) x inner spmd(NUM_QK_CORES): do not fold tiles into one fat SPMD.
    cmp_block_num = pl.tensor.dim(cmp_kv, 0)
    cmp_kv_flat = pl.reshape(cmp_kv, [cmp_block_num * CMP_STORAGE_BLOCK_SIZE, HEAD_DIM])
    q_flat = pl.reshape(q, [T * H, HEAD_DIM])
    sparse_blk_mi = pl.create_tensor(
        [T * (H // QK_STORE_H_TILE) * SPARSE_BLOCKS * QK_STORE_H_TILE, 1], dtype=pl.FP32
    )
    sparse_blk_li = pl.create_tensor(
        [T * (H // QK_STORE_H_TILE) * SPARSE_BLOCKS * QK_STORE_H_TILE, 1], dtype=pl.FP32
    )
    sparse_blk_oi = pl.create_tensor(
        [T * (H // QK_STORE_H_TILE) * SPARSE_BLOCKS * QK_STORE_H_TILE, HEAD_DIM], dtype=pl.FP32
    )

    for tile_i in pl.range(N_BATCH_TILES):
        with pl.spmd(NUM_QK_CORES, name_hint="qk_pv", deps=[qk_plan_tid]) as qk_tid:
            ...
            qk_lane_iters = (QK_ITEMS_TILE - qk_core + NUM_QK_CORES - 1) // NUM_QK_CORES
            for qk_it in pl.range(qk_lane_iters):
                qk_flat = qk_core + qk_it * NUM_QK_CORES
                qk_item = qk_flat
                qk_t_local = qk_item // SPARSE_BLOCKS
                qk_sb = qk_item - qk_t_local * SPARSE_BLOCKS
                qk_t = t_base + qk_t_local
                qk_b = qk_t // S
                qk_token_base = qk_t * (H // QK_STORE_H_TILE) * SPARSE_BLOCKS * QK_STORE_H_TILE
                ...
                if qk_block_valid > 0:
                    ...
                    # Each QK/PV result is stored in cache-line-sized head groups.
                    for qk_hb in pl.pipeline(H // QK_M_TILE, stage=2):
                        ...
                        for qk_sub in pl.unroll(QK_M_TILE // QK_STORE_H_TILE):
                            qk_h_idx = qk_hb * (QK_M_TILE // QK_STORE_H_TILE) + qk_sub
                            qk_r0 = qk_sub * QK_STORE_H_TILE
                            qk_blk_base = qk_token_base + qk_h_idx * SPARSE_BLOCKS * QK_STORE_H_TILE
                            qk_row = qk_blk_base + qk_sb * QK_STORE_H_TILE
                            sparse_blk_mi[qk_row : qk_row + QK_STORE_H_TILE, 0 : 1] = qk_mi[
                                qk_r0 : qk_r0 + QK_STORE_H_TILE, 0 : 1
                            ]
                            sparse_blk_li[qk_row : qk_row + QK_STORE_H_TILE, 0 : 1] = qk_li[
                                qk_r0 : qk_r0 + QK_STORE_H_TILE, 0 : 1
                            ]
                            sparse_blk_oi[qk_row : qk_row + QK_STORE_H_TILE, 0 : HEAD_DIM] = qk_oi[
                                qk_r0 : qk_r0 + QK_STORE_H_TILE, 0 : HEAD_DIM
                            ]
                else:
                    qk_mi_invalid_row = pl.full([1, QK_STORE_H_TILE], dtype=pl.FP32, value=-3.0e38)
                    qk_li_invalid_row = pl.full([1, QK_STORE_H_TILE], dtype=pl.FP32, value=0.0)
                    qk_mi_invalid = pl.reshape(qk_mi_invalid_row, [QK_STORE_H_TILE, 1])
                    qk_li_invalid = pl.reshape(qk_li_invalid_row, [QK_STORE_H_TILE, 1])
                    qk_oi_zero = pl.full([QK_STORE_H_TILE, HEAD_DIM], dtype=pl.FP32, value=0.0)
                    for qk_h_idx in pl.range(H // QK_STORE_H_TILE):
                        qk_blk_base = qk_token_base + qk_h_idx * SPARSE_BLOCKS * QK_STORE_H_TILE
                        qk_row = qk_blk_base + qk_sb * QK_STORE_H_TILE
                        sparse_blk_mi[qk_row : qk_row + QK_STORE_H_TILE, 0 : 1] = qk_mi_invalid
                        sparse_blk_li[qk_row : qk_row + QK_STORE_H_TILE, 0 : 1] = qk_li_invalid
                        sparse_blk_oi[qk_row : qk_row + QK_STORE_H_TILE, 0 : HEAD_DIM] = qk_oi_zero
    ...
    # j^1 lane-swap index for merge_norm's rotation gather. Shaped [MERGE_H_TILE, ROPE_DIM]
    # because gather's index must match its source rows.
    rope_swap_idx = pl.create_tensor([MERGE_H_TILE, ROPE_DIM], dtype=pl.INT32)
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="rope_cs_swap"):
        sw_ones = pl.full([MERGE_H_TILE, ROPE_DIM], dtype=pl.FP32, value=1.0)
        ...
        rope_swap_idx[0:MERGE_H_TILE, 0:ROPE_DIM] = pl.cast(sw_swap_f, target_type=pl.INT32)

    with pl.spmd(N_BATCH_TILES, name_hint="rope_cs"):
        rope_tile = pl.tile.get_block_idx()
        rope_t0 = rope_tile * T_TILE_BATCH
        cs_ones = pl.full([T_TILE_BATCH, ROPE_DIM], dtype=pl.FP32, value=1.0)
        ...
        cs_cos = pl.cast(freqs_cos[rope_t0:rope_t0 + T_TILE_BATCH, 0:HALF_ROPE], target_type=pl.FP32)
        cs_sin = pl.cast(freqs_sin[rope_t0:rope_t0 + T_TILE_BATCH, 0:HALF_ROPE], target_type=pl.FP32)
        rope_cos_il[rope_t0:rope_t0 + T_TILE_BATCH, 0:ROPE_DIM] = pl.gather(cs_cos, dim=-1, index=cs_dup_idx)
        cs_sin_il = pl.gather(cs_sin, dim=-1, index=cs_dup_idx)
        rope_sin_signed[rope_t0:rope_t0 + T_TILE_BATCH, 0:ROPE_DIM] = pl.mul(cs_sin_il, cs_sign)
    ...
    merge_tids = pl.array.create(N_BATCH_TILES, pl.TASK_ID)
    for tile_i in pl.range(N_BATCH_TILES):
        with pl.spmd(64, name_hint="merge_norm") as merge_tid:
            m_idx_lane = pl.tile.get_block_idx()
            t_base = tile_i * T_TILE_BATCH
            n_heads_t = H // MERGE_H_TILE
            for m_idx in pl.range(m_idx_lane, MERGE_ITEMS_TILE, 64):
                m_t_local = m_idx // n_heads_t
                m_h_idx = m_idx - m_t_local * n_heads_t
                m_t = t_base + m_t_local
                m_h0 = m_h_idx * MERGE_H_TILE
                m_store_h_idx = m_h_idx // (QK_STORE_H_TILE // MERGE_H_TILE)
                m_store_h_offset = (m_h_idx % (QK_STORE_H_TILE // MERGE_H_TILE)) * MERGE_H_TILE
                m_token_base = m_t * (H // QK_STORE_H_TILE) * SPARSE_BLOCKS * QK_STORE_H_TILE
                m_blk_base = m_token_base + m_store_h_idx * SPARSE_BLOCKS * QK_STORE_H_TILE + m_store_h_offset
                ...
                for m_sb in pl.pipeline(1, SPARSE_BLOCKS, stage=2):
                    m_row = m_blk_base + m_sb * QK_STORE_H_TILE
                    m_cur_mi = sparse_blk_mi[m_row : m_row + MERGE_H_TILE, 0 : 1]
                    m_cur_li = sparse_blk_li[m_row : m_row + MERGE_H_TILE, 0 : 1]
                    m_cur_oi = sparse_blk_oi[m_row : m_row + MERGE_H_TILE, 0 : HEAD_DIM]
                ...
                for n_hi in pl.unroll(MERGE_H_TILE):
    ...
    proj_b_tids = pl.array.create(O_GROUPS * N_BATCH_TILES, pl.TASK_ID)
        ...
        for g in pl.parallel(O_GROUPS):
            ...
            for tile_i in pl.range(N_BATCH_TILES):
                with pl.spmd(O_LORA // PROJ_A_MM_N_TILE, name_hint="proj_a_mm",
                             deps=[merge_tids[tile_i]]) as pa_tid:
                    nf_pa = pl.tile.get_block_idx()
                    pa_n0 = nf_pa * PROJ_A_MM_N_TILE
                    t_base_pa = tile_i * T_TILE_BATCH
                    for row0_local in pl.range(0, T_TILE_BATCH, MM_T_TILE):
                        row0 = t_base_pa + row0_local
                        n_rows = pl.min(MM_T_TILE, T_TILE_BATCH - row0_local)
                        acc_a = pl.create_tensor([1, MM_T_TILE, PROJ_A_MM_N_TILE], dtype=pl.FP32)
                        for kb in pl.pipeline(0, O_GROUP_IN // A_K_TILE, stage=2):
                            k0 = kb * A_K_TILE
                            xa_k_chunk = pl.slice(o_packed, [MM_T_TILE, A_K_TILE], [row_base_o + row0, k0], valid_shape=[n_rows, A_K_TILE])
                            wa_k_chunk = wo_a[g : g + 1, pa_n0 : pa_n0 + PROJ_A_MM_N_TILE, k0 : k0 + A_K_TILE]
                            acc_a = pl.matmul_acc(acc_a, xa_k_chunk, wa_k_chunk, b_trans=True, init_cond=(kb == 0))
                        o_r_pad = pl.assemble(o_r_pad, acc_a, [row0, out_col_g + pa_n0])

                with pl.spmd(1, name_hint="quant", deps=[pa_tid]) as q_tid:
                    # spmd(1) requires get_block_idx; value is always 0 — add into qt only.
                    bid = pl.tile.get_block_idx()
                    t_base_q = tile_i * T_TILE_BATCH
                    for qt_local in pl.pipeline(0, T_TILE_BATCH, QUANT_TOKEN_TILE, stage=2):
                        qt = t_base_q + qt_local + bid
                ...
                with pl.spmd(D // PROJ_B_D_TILE, name_hint="proj_b_mm", deps=[q_tid]) as pb_tid:
                    dc = pl.tile.get_block_idx()
                    d0 = dc * PROJ_B_D_TILE
                    t_base_pb = tile_i * T_TILE_BATCH
                    for row0_local in pl.range(0, T_TILE_BATCH, MM_T_TILE):
                        row0 = t_base_pb + row0_local
                        n_rows = pl.min(MM_T_TILE, T_TILE_BATCH - row0_local)
                        for nf_pb in pl.range(PROJ_B_D_TILE // PROJ_B_MM_N_TILE):
                            pb_n0 = d0 + nf_pb * PROJ_B_MM_N_TILE
                            ...
                            for kb in pl.pipeline(0, O_LORA // B_K_TILE, stage=2):
                                ...
                                b_act = pl.slice(
                                    ...
                                    [MM_T_TILE, B_K_TILE],
                                    [row0, k0],
                                    valid_shape=[n_rows, B_K_TILE],
                                )
                                b_weight = wo_b[pb_n0 : pb_n0 + PROJ_B_MM_N_TILE, k0 : k0 + B_K_TILE]
                                acc_b = pl.matmul_acc(acc_b, b_act, b_weight, b_trans=True, init_cond=(kb == 0))
                            partials = pl.assemble(partials, acc_b, [row0, g * D + pb_n0])
                proj_b_tids[g * N_BATCH_TILES + tile_i] = pb_tid
    ...
    for tile_i in pl.range(N_BATCH_TILES):
        with pl.spmd(8, name_hint="proj_b_act",
                     deps=[proj_b_tids[i] for i in range(O_GROUPS * N_BATCH_TILES)]) as _act_tid:
            act_idx_lane = pl.tile.get_block_idx()
            t_base = tile_i * T_TILE_BATCH
            n_act = (D // PROJ_B_ACT_N_TILE) * PROJ_B_ACT_T_BLOCKS_TILE
            for act_idx in pl.range(act_idx_lane, n_act, 8):
                nreg = act_idx // PROJ_B_ACT_T_BLOCKS_TILE
                tblk = act_idx - nreg * PROJ_B_ACT_T_BLOCKS_TILE
                ob_n0 = nreg * PROJ_B_ACT_N_TILE
                t0 = t_base + tblk * PROJ_B_ACT_TASK_T_TILE
