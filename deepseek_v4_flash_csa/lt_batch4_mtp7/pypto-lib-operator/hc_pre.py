...
from config import FLASH as M, DECODE_BATCH, DECODE_SEQ, PREFILL_BATCH, PREFILL_SEQ, N_BATCH_TILES, T_TILE_BATCH
...
def hc_pre(
    ...
    n_hc_rms_tile = T_TILE_BATCH // T_TILE
    for t_lane in pl.spmd(N_BATCH_TILES, name_hint="hc_pre_rms"):
        tile_i = t_lane
        t_base = tile_i * T_TILE_BATCH
        for t in pl.range(n_hc_rms_tile):
            t0 = t_base + t * T_TILE
    ...
    n_linear_t_tile = (T_TILE_BATCH + LINEAR_T_TILE - 1) // LINEAR_T_TILE
    n_linear_items_tile = n_linear_t_tile * LINEAR_OK
    for tile_i in pl.range(N_BATCH_TILES):
        for core in pl.spmd(4, name_hint="hc_pre_linear"):
            t_base = tile_i * T_TILE_BATCH
            for task_local in pl.range(core, n_linear_items_tile, 4):
                t0_local = (task_local // LINEAR_OK) * LINEAR_T_TILE
                t0 = t_base + t0_local
                linear_split = task_local % LINEAR_OK
                k_base = linear_split * LINEAR_K_PER_SPLIT
                t_rows = pl.min(LINEAR_T_TILE, T_TILE_BATCH - t0_local)
                ...
                mixes_partials = pl.assemble(mixes_partials, acc, [partial_row0, 0])

    # Partials are reduced in ascending K order.
    mixes_raw = pl.create_tensor([t_linear, MIX_PAD], dtype=pl.FP32)
    for linear_block_lane in pl.spmd(N_BATCH_TILES, name_hint="hc_pre_linear_reduce"):
        tile_i = linear_block_lane
        t_base = tile_i * T_TILE_BATCH
        for linear_block in pl.range(n_linear_t_tile):
            linear_t0 = t_base + linear_block * LINEAR_T_TILE
    ...
    n_split_t = T_TILE_BATCH // T_TILE
    for ob_lane in pl.spmd(N_BATCH_TILES, name_hint="split_pre_post"):
        tile_i = ob_lane
        t_base = tile_i * T_TILE_BATCH
        for ob in pl.range(n_split_t):
            t0 = t_base + ob * T_TILE
    ...
    n_comb_t = T_TILE_BATCH // COMB_T_TILE
    for ob_lane in pl.spmd(N_BATCH_TILES, name_hint="comb_sinkhorn"):
        tile_i = ob_lane
        t_base = tile_i * T_TILE_BATCH
        for ob in pl.range(n_comb_t):
            t0 = t_base + ob * COMB_T_TILE
    ...
    n_mix_d = D // D_SPMD
    n_mix_items_tile = n_split_t * n_mix_d
    for tile_i in pl.range(N_BATCH_TILES):
        for core in pl.spmd(4, name_hint="mix_x"):
            t_base = tile_i * T_TILE_BATCH
            for blk_local in pl.range(core, n_mix_items_tile, 4):
                t0 = t_base + (blk_local // n_mix_d) * T_TILE
                d_base = (blk_local % n_mix_d) * D_SPMD
