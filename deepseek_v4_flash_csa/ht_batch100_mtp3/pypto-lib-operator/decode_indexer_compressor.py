...
RMS_PAD_ROWS = ((B + RMS_PAD_TILE - 1) // RMS_PAD_TILE) * RMS_PAD_TILE
...
def indexer_compressor(
    ...
    with pl.spmd(
        OUT_DIM // PROJ_OUT_TILE, name_hint="kv_score_proj", deps=[late_dep]
    ) as _kv_score_tid:
        idx = pl.tile.get_block_idx()
        o0 = (idx % (OUT_DIM // PROJ_OUT_TILE)) * PROJ_OUT_TILE
        for global_row0 in pl.range(0, BS_PAD, MM_B_TILE):
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="rmsnorm_rope"):
        # Process bounded 16-row tiles, masking the final batch tail.
        ...
        for batch0 in pl.range(0, RMS_PAD_ROWS, RMS_PAD_TILE):
            cos_b = pl.slice(cos, [RMS_PAD_TILE, ROPE_HEAD_DIM], [batch0, 0], valid_shape=[pl.min(RMS_PAD_TILE, B - batch0), ROPE_HEAD_DIM])
            sin_b = pl.slice(sin, [RMS_PAD_TILE, ROPE_HEAD_DIM], [batch0, 0], valid_shape=[pl.min(RMS_PAD_TILE, B - batch0), ROPE_HEAD_DIM])
            partial_sq = pl.full([1, RMS_PAD_TILE], dtype=pl.FP32, value=0.0)
            for k0 in pl.pipeline(0, HEAD_DIM, HEAD_TILE, stage=2):
                kv_rms_chunk = pooled_kv[batch0 : batch0 + RMS_PAD_TILE, k0 : k0 + HEAD_TILE]
            ...
            for k0 in pl.pipeline(0, NOPE_HEAD_DIM, HEAD_TILE, stage=2):
                kv_norm_chunk = pooled_kv[batch0 : batch0 + RMS_PAD_TILE, k0 : k0 + HEAD_TILE]
                gamma = pl.cast(norm_w_2d[:, k0 : k0 + HEAD_TILE], pl.FP32)
                normed_chunk = pl.col_expand_mul(pl.row_expand_mul(kv_norm_chunk, inv_rms), gamma)
                normed_kv[batch0 : batch0 + RMS_PAD_TILE, k0 : k0 + HEAD_TILE] = pl.cast(
                    normed_chunk,
                    target_type=pl.BF16,
                    mode="rint",
                )

            kv_rope_norm = pooled_kv[batch0 : batch0 + RMS_PAD_TILE, NOPE_HEAD_DIM : HEAD_DIM]
            ...
            normed_kv[batch0 : batch0 + RMS_PAD_TILE, NOPE_HEAD_DIM : HEAD_DIM] = pl.cast(
                rope_rot,
                target_type=pl.BF16,
                mode="rint",
            )
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="kv_hadamard"):
        for batch0 in pl.range(0, RMS_PAD_ROWS, RMS_PAD_TILE):
            kv_proj_tile = normed_kv[batch0 : batch0 + RMS_PAD_TILE, 0 : HEAD_DIM]
            for o0 in pl.range(0, HEAD_DIM, OUT_TILE):
                ...
                kv_final[batch0 : batch0 + RMS_PAD_TILE, o0 : o0 + OUT_TILE] = kv_hadamard_acc
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="kv_and_cache_write"):
        ...
        for batch0 in pl.range(0, RMS_PAD_ROWS, RMS_PAD_TILE):
            kv_blk_f32 = pl.cast(
                pl.cast(kv_final[batch0 : batch0 + RMS_PAD_TILE, 0 : HEAD_DIM], target_type=pl.BF16, mode="rint"),
                target_type=pl.FP32)
            ...
            for inner in pl.range(pl.min(RMS_PAD_TILE, B - batch0)):
                c_idx = batch0 + inner
                first_pos_b = pl.read(position_ids, [c_idx, 0])
                pos_b = first_pos_b % COMPRESS_RATIO
                if pos_b + S >= COMPRESS_RATIO:
