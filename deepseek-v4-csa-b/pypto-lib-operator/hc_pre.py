...
def hc_pre(
    ...
    for t_lane in pl.spmd(1, name_hint="hc_pre_rms"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for t in pl.range(t_lane, t_dim // T_TILE, 1):
            t0 = t * T_TILE
    ...
    for task_lane in pl.spmd(4, name_hint="hc_pre_linear"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for task in pl.range(task_lane, (t_linear // LINEAR_T_TILE) * LINEAR_OK, 4):
    ...
    for linear_block_lane in pl.spmd(1, name_hint="hc_pre_linear_reduce"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for linear_block in pl.range(linear_block_lane, t_linear // LINEAR_T_TILE, 1):
    ...
    for ob_lane in pl.spmd(1, name_hint="split_pre_post"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for ob in pl.range(ob_lane, t_dim // T_TILE, 1):
            t0 = ob * T_TILE
    ...
    for ob_lane in pl.spmd(1, name_hint="comb_sinkhorn"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for ob in pl.range(ob_lane, t_dim // COMB_T_TILE, 1):
            t0 = ob * COMB_T_TILE
    ...
    for blk_lane in pl.spmd(4, name_hint="mix_x"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for blk in pl.range(blk_lane, (t_dim // T_TILE) * (D // D_SPMD), 4):
