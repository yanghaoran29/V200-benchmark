...
ATTN_K_TILE = 128
NUM_QK_CORES = 100        # qk_pv dispatch lanes
A_K_TILE = 256           # proj_a cube K frag
PROJ_A_MM_N_TILE = 64    # proj_a cube N frag
MM_T_TILE = 16
...
QUANT_TOKEN_TILE = 8
PROJ_B_D_TILE = 256      # proj_b_mm D chunk per task; its N frags loop inside the task
...
def sparse_attn_csa(
    ...
    with pl.spmd(64, name_hint="merge_norm") as merge_tid:
        ...
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for m_idx in pl.range(m_idx_lane, T * (H // MERGE_H_TILE), 64):
    ...
    with pl.manual_scope():
        for g in pl.parallel(O_GROUPS):
            ...
            with pl.spmd(O_LORA // PROJ_A_MM_N_TILE, name_hint="proj_a_mm", deps=[merge_tid]) as pa_tid:
                nf = pl.tile.get_block_idx()
                n0 = nf * PROJ_A_MM_N_TILE
                for row0 in pl.range(0, T_PAD, MM_T_TILE):
                    acc_a = pl.create_tensor([1, MM_T_TILE, PROJ_A_MM_N_TILE], dtype=pl.FP32)
                    for kb in pl.pipeline(0, O_GROUP_IN // A_K_TILE, stage=2):
                        k0 = kb * A_K_TILE
                        xa_k_chunk = pl.slice(o_packed, [MM_T_TILE, A_K_TILE], [row_base_o + row0, k0], valid_shape=[pl.min(MM_T_TILE, T - row0), A_K_TILE])
                    ...
                    o_r_pad = pl.assemble(o_r_pad, acc_a, [row0, out_col_g + n0])

            col_g = g * O_LORA
            with pl.at(level=pl.Level.CORE_GROUP, name_hint="quant", deps=[pa_tid]) as q_tid:
    ...
    with pl.spmd(8, name_hint="proj_b_act",
        ...
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for act_idx in pl.range(act_idx_lane, (D // PROJ_B_ACT_N_TILE) * (T // PROJ_B_ACT_TASK_T_TILE), 8):
