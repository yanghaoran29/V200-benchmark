...
# Each token retains five disjoint cache-page splits (200 work items in total).
# Dispatch 100 mixed blocks; each block traverses two work items sequentially.
REDUCE_NSPLIT = 5
SCORE_BLOCKS = 100  # Each lane processes two independent (token, page-split) items.
Q_TILE = 256
...
def indexer(
    ...
    for ot in pl.spmd(IDX_N_HEADS * IDX_HEAD_DIM // Q_OUT_TILE, name_hint="idx_qr_proj_matmul"):
        ...
        for row0 in pl.range(0, T_PAD, MM_ROW_TILE):
            for ns in pl.range(0, Q_OUT_TILE, MM_N_TILE):
                ...
                for kb in pl.pipeline(0, Q_LORA // Q_TILE, stage=2):
                    q0 = kb * Q_TILE
                    qr_tile = pl.slice(qr, [MM_ROW_TILE, Q_TILE], [row0, q0], valid_shape=[pl.min(MM_ROW_TILE, T - row0), Q_TILE])
                    wq_tile = wq_b[q0 : q0 + Q_TILE, o_base + ns : o_base + ns + MM_N_TILE]
                    qr_acc = pl.matmul_acc(qr_acc, qr_tile, wq_tile, init_cond=(q0 == 0))
                qr_acc_pad[row0:row0 + MM_ROW_TILE, o_base + ns : o_base + ns + MM_N_TILE] = qr_acc
    ...
    for idx_lane in pl.spmd(16, name_hint="qr_rope"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for idx in pl.range(idx_lane, T * IDX_N_HEADS // ROPE_ROW_TILE, 16):
    ...
    for idx_lane in pl.spmd(32, name_hint="qr_hadamard_matmul"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for idx in pl.range(idx_lane, T * IDX_N_HEADS // QH_MM_TILE, 32):
            o0 = idx * QH_MM_TILE
    ...
    for idx_lane in pl.spmd(8, name_hint="qr_hadamard_quant"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for idx in pl.range(idx_lane, T * IDX_N_HEADS // QH_QUANT_TILE, 8):
    ...
    with pl.spmd(WEIGHTS_OK, name_hint="weights_proj", deps=[late_dep]) as _weights_tid:
        ...
        for row0 in pl.range(0, T_PAD, MM_ROW_TILE):
            ...
            for db in pl.range(WEIGHTS_K_SLICE // D_TILE):
                d0 = k_base + db * D_TILE
                x_tile = pl.slice(x_flat, [MM_ROW_TILE, D_TILE], [row0, d0], valid_shape=[pl.min(MM_ROW_TILE, T - row0), D_TILE])
                weights_proj_tile = weights_proj[d0 : d0 + D_TILE, :]
                weights_acc = pl.matmul_acc(weights_acc, x_tile, weights_proj_tile, init_cond=(db == 0))
            weights_partial[kb * T_PAD + row0 : kb * T_PAD + row0 + MM_ROW_TILE, :] = weights_acc
    ...
    for score_lane in pl.spmd(SCORE_BLOCKS, name_hint="score",
                        optimizations=[pl.split(pl.SplitMode.NONE, slot_num=2)]):
        for unit in pl.range(score_lane, T * REDUCE_NSPLIT, SCORE_BLOCKS):
    ...
    for t_lane in pl.spmd(8, name_hint="topk"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for t in pl.range(t_lane, T, 8):
