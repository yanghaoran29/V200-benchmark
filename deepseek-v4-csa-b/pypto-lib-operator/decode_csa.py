...
def attention_csa(
    ...
    # All projection branches depend directly on the real RMS producer.
    late_dep = rms_tid
    ...
    for wb_blk_lane in pl.spmd(1, name_hint="csa_cache_writeback"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for wb_blk in pl.range(wb_blk_lane, T // CSA_WB_TOKEN_TILE, 1):
...
def build_tensor_specs(start_pos=None):
    ...
    def allocate_pages(page_span, table_cols):
        counts = (fixture_starts.to(torch.int64) + S + page_span - 1) // page_span
        table = torch.full((B, table_cols), -1, dtype=torch.int32)
        cursor = 0
        for request, count in enumerate(counts.tolist()):
            if count > table_cols:
                raise ValueError("Scheme B fixture exceeds logical page-table capacity")
            table[request, :count] = torch.arange(cursor, cursor + count, dtype=torch.int32)
            cursor += count
        return table, cursor

    main_table, MAIN_STATE_BLOCK_NUM = allocate_pages(MAIN_STATE_BLOCK_SIZE, MAIN_STATE_MAX_BLOCKS)
    inner_table, INNER_STATE_BLOCK_NUM = allocate_pages(INNER_STATE_BLOCK_SIZE, INNER_STATE_MAX_BLOCKS)
    ori_table, ORI_BLOCK_NUM = allocate_pages(BLOCK_SIZE, ORI_TABLE_MAX_BLOCKS)
    cmp_table, CMP_BLOCK_NUM = allocate_pages(CMP_STORAGE_BLOCK_SIZE * COMPRESS_RATIO, CMP_MAX_BLOCKS)
    idx_table, IDX_CACHE_BLOCK_NUM = allocate_pages(CMP_STORAGE_BLOCK_SIZE * COMPRESS_RATIO, IDX_CACHE_MAX_BLOCKS)
    ...
    def init_idx_block_table():
        return idx_table.clone()

    def init_attn_sink():
        return torch.ones(H) * 4.0

    def init_start_pos():
        return fixture_starts.clone()
...
if __name__ == "__main__":
    ...
    result = run(
        ...
        config=dict(
            ...
            save_kernels_dir=args.dep_output_dir,
        ),
        rtol=1e-2,
        atol=1e-2,
        compare_fn={
