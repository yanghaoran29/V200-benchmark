...
QR_OK = 8               # qr_proj split-K factor          | D//QR_OK cores share each N-group
QR_K_SLICE = D // QR_OK # qr_proj K per split (=512)      | QR_K_SLICE//QR_K_TILE inner chunks
...
KV_K_SLICE = D // KV_OK # kv_proj K per split (=512)      | KV_K_SLICE//KV_K_TILE inner chunks
...
def rope_prepare(
    ...
    for qrp_idx_lane in pl.spmd(1, name_hint="q_rope_prepare"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for qrp_idx in pl.range(qrp_idx_lane, token_tiles, 1):
...
def qkv_proj_rope(
    ...
    # Two passes per block: pass 1 computes amax; pass 2 recomputes norm and quantizes.
    for tg_idx_lane in pl.spmd(1, name_hint="qr_rms_norm_quant"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for tg_idx in pl.range(tg_idx_lane, t_dim // T_TILE, 1):
            tg = tg_idx * T_TILE
            qr_sq_sum = pl.full([1, T_TILE], dtype=pl.FP32, value=0.0)
            qr_amax_g = pl.full([1, T_TILE], dtype=pl.FP32, value=0.0)
    ...
    for qproj_n_idx in pl.spmd((H * HEAD_DIM) // QPROJ_MM_N_TILE, name_hint="qproj_matmul"):
        ...
        for tc in pl.range(t_matmul // QPROJ_M_TILE):
            ...
            for qb in pl.pipeline(0, Q_LORA // Q_PROJ_TILE, stage=2):
                ...
                qr_i8_chunk = pl.slice(qr_i8_matmul, [QPROJ_M_TILE, Q_PROJ_TILE], [t0, qr_proj_col0], valid_shape=[pl.min(QPROJ_M_TILE, t_dim - t0), Q_PROJ_TILE])
                wq_chunk = wq_b[qr_proj_col0 : qr_proj_col0 + Q_PROJ_TILE, w_col0 : w_col0 + QPROJ_MM_N_TILE]
                col_acc = pl.matmul_acc(col_acc, qr_i8_chunk, wq_chunk, init_cond=(qr_proj_col0 == 0))
    ...
    # RoPE: out[j] = inv_rms * (x[j] * cos[j] + x[j^1] * sign[j] * sin[j]).
    q_flat = pl.reshape(q, [t_dim, H * HEAD_DIM])
    with pl.spmd(H // Q_ROPE_H_TILE, name_hint="qproj_dequant_rms_nope_rope") as q_rope_tid:
    ...
    for tg_idx_lane in pl.spmd(1, name_hint="kv_rms_norm_rope"):
        # Scheme B: retain dispatch width; traverse additional work inside each block.
        for tg_idx in pl.range(tg_idx_lane, t_dim // KV_RMS_T_TILE, 1):
            tg = tg_idx * KV_RMS_T_TILE
