...
void aicpu_orchestration_entry(const ChipStorageTaskArgs& orch_args) {
    ...
    PTO2_SCOPE(PTO2ScopeMode::MANUAL) {
        ...
        for (int64_t b0 = 0; b0 < batch_padded; b0 += 16) {
            ...
            // Non-SPMD: one mixed single-task per output column chunk (HIDDEN/128 = 40),
            // chunk index passed as a scalar; each task shards the C2V GM pipe by its chunk.
            std::vector<PTO2TaskId> out_proj_task_ids;
            out_proj_task_ids.reserve(40);
            for (int64_t oc = 0; oc < 40; ++oc) {
                ...
                out_proj_task_ids.push_back(__rt_op.task_id());
            }

            // Task 12: post_rmsnorm — depends on every out_proj chunk (full resid1_tile read).
            ...
            for (const PTO2TaskId& __op_tid : out_proj_task_ids) {
                params_t11.add_dep(__op_tid);
            }
            ...
            for (int64_t dob = 0; dob < 40; dob += 1) {
                ...
                // Task 16: down_proj — reads full mlp_tile, must wait for silu (single SPMD launch).
                ...
                // Task 17: down_proj_residual — needs this chunk's down_proj output and the
                // out_proj chunk that produced the matching resid1_tile columns.
                ...
                params_t16.add_dep(out_proj_task_ids[static_cast<size_t>(dob)]);
